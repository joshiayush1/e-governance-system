        ...
        <div className="navbar h-24 w-full text-white flex justify-between">
          <div className="logo flex flex-col justify-center items-center h-full md:w-1/5 pl-5 ">
            ...
            <p className="font-extrabold text-[10px] cursor-pointer text-amber-500">
              <Link to="/">PAY YOUR FEES</Link>
            </p>
          </div>
          <ul className="menuItems lg:flex justify-evenly items-center h-full lg:w-3/6 font-bold hidden text-slate-700">
            <li className="hover:text-amber-500 cursor-pointer duration-700 px-5">
              <Link to="/">HOME</Link>
            </li>
            {role === "Admin" ? (
              <h1 className="hover:text-amber-500 cursor-pointer duration-700 px-5">
                <Link to="/student-payments">PAYMENTS</Link>
              </h1>
            ) : role === "Student" ? (
              <h1 className="hover:text-amber-500 cursor-pointer duration-700 px-5">
                <Link to="/institute-fees">EDUCATION FEES</Link>
              </h1>
            ) : (
              <h1 className="hover:text-amber-500 cursor-pointer duration-700 px-5">
                EDUCATION FEES
            ...
            {toggleMenu && (
              <div className="w-4/6 h-[300px] fixed bg-teal-100 mt-[360px] mr-24 z-50 md:hidden">
                <div className="h-full w-full flex flex-col text-slate-700 font-medium text-xl gap-4">
                  <h1 className="hover:text-amber-500 cursor-pointer duration-700 px-5 pt-3">
                    <Link to="/">HOME</Link>
                  </h1>
                  <hr className="border" />
                  {role === "Admin" ? (
